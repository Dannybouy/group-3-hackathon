...
public interface TransactionRepository
    ...
    /**
     * Fetches all transactions for a specific account and routing number
     * that occurred between the given start and end dates.
     */
    @Query("SELECT t FROM Transaction t "
        + " WHERE ((t.fromAccountNum = ?1 AND t.fromRoutingNum = ?2) "
        + "   OR (t.toAccountNum = ?1 AND t.toRoutingNum = ?2)) "
    ...
    List<Transaction> findTransactionsForDateRange(String accountNum, String routingNum, Date startDate, Date endDate, Pageable pageable);

    /**
     * Fetches the balance of an account as of a specific date.
     */
    @Query(value = "SELECT " +
